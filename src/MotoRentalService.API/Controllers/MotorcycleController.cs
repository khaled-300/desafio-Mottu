...
namespace MotoRentalService.API.Controllers
{
    ...
    public class MotorcycleController : ControllerBase
    {
        ...
        public MotorcycleController(IMediator mediator, ILogger<MotorcycleController> logger)
        {
            _mediator = mediator;
            _logger = logger;
        }
        ...
        /// <param name="licensePlate">The new license plate information for the motorcycle.</param>
        /// <param name="cancellationToken">A token to monitor for cancellation requests.</param>
        /// <returns>An <see cref="ActionResult"/> indicating the result of the update operation.</returns>
        [HttpPut("{motorcycleId}/{licensePlate}")]
        [Consumes("application/json")]
        [Produces("application/json")]
        [ProducesResponseType(typeof(MotorcycleCommandResult), StatusCodes.Status200OK)]
        [ProducesResponseType(typeof(MotorcycleCommandResult), StatusCodes.Status400BadRequest)]
        [ProducesResponseType(StatusCodes.Status500InternalServerError)]
        public async Task<IActionResult> UpdateMotorcycleAsync(int motorcycleId, string licensePlate, CancellationToken cancellationToken)
        {
            try
            {
